...
class Agent {
  constructor(store,{launch,origin=ORIGIN,slotParser=slotFromHref,headless=true,browserChannel=process.platform==='win32'?'chrome':undefined,clock=()=>Date.now()}={}) {
    ...
    this.info={phase:'idle',message:'Tayyor',startedAt:null};this.lastCalendarError=0;
    this.headless=headless;this.browserChannel=browserChannel;this.clock=clock;this.runId=null;this.endAt=Infinity;this.seenSlots=new Set();this.evidence={};this.slotUse=new Map();this.warmPool=new Map();
  }
  status(){return {...this.info,running:this.running,active:this.tasks.size};}
  ...
  start(options){
    ...
    this.stopping=false;this.running=true;this.gate=new Gate(150);this.slotUse=new Map();this.warmPool=new Map();
  ...
  async makePage(context) {
    ...
    page._health={status:200,retryAfter:null};
    page.on('response',r=>{
  ...
  async navigation(page,action){
    ...
    while(true){
      this.check();await this.gate.take(()=>this.check());page._health={status:200};
      try{
  ...
  async calendar(page,month){
    ...
    for(const day of days){
      ...
      await this.gate.take(()=>this.check());page._health={status:200};
  ...
  async run(){
    ...
    try{
      ...
      const context=await this.resource(()=>this.browser.newContext({locale:'en-US'}),c=>c.close());calendar=await this.makePage(context);
      // Prepare isolated blank sessions concurrently before the release window.
      await Promise.all(this.store.list().filter(c=>c.state==='queued').slice(0,this.options.parallel).map(async candidate=>{
        let ctx;try{ctx=await this.resource(()=>this.browser.newContext({locale:'en-US'}),c=>c.close());const page=await this.makePage(ctx);this.warmPool.set(candidate.id,{context:ctx,page});}
        catch(e){if(ctx)await ctx.close().catch(()=>{});if(e instanceof Stop)throw e;this.store.event('Oldindan sessiya ochilmadi; joy topilganda yana uriniladi',candidate.id);}
      }));
      this.store.notify(`prepared:${this.runId}`,'🟢 Agent tayyorlanmoqda. Short stay. Bron qidiruvi '+new Date(this.options.startAt).toLocaleTimeString('uz-UZ',{timeZone:'Asia/Tashkent'})+' da boshlanadi (Toshkent).');
      // Open a warm session without booking before the requested start.
      if(Date.now()<this.options.startAt){
        while(Date.now()<this.options.startAt){
          try{await this.goto(calendar,`${this.origin}/reservations/calendar?category=12`);break;}
          catch(e){if(!(e instanceof Temporary))throw e;await this.pause(3000+Math.random()*2000);}
        }
      }
      while(Date.now()<this.options.startAt){
        this.info.phase='scheduled';this.info.message=`Boshlanish: ${new Date(this.options.startAt).toLocaleString('uz-UZ',{timeZone:'Asia/Tashkent'})} (Toshkent)`;
        await this.pause(Math.min(1000,this.options.startAt-Date.now()));
      }
      this.info.phase='watching';this.info.message='Short stay: barcha ochilgan sanalar kuzatilmoqda';
      let monthIndex=0;
      while(true){
        this.check();
        const queued=this.store.list().filter(c=>c.state==='queued'&&!this.tasks.has(c.id));
        if(!queued.length&&this.tasks.size){await this.pause(500);continue;}
        if(!queued.length&&!this.options.allowMonitorOnly)break;
        if(this.tasks.size>=this.options.parallel){await this.pause(300);continue;}
        try{
          const slots=await this.calendar(calendar,months()[monthIndex++%2]);
          let openedShot;
          for(const slot of slots){
            if(this.seenSlots.has(slot.url))continue;this.seenSlots.add(slot.url);this.stats.found++;
            if(openedShot===undefined)openedShot=await this.capture(calendar,'available');
            this.store.notify(`opened:${this.runId}:${slot.date}:${slot.time}` ,`🟢 SHORT STAY JOY OCHILDI\n${slot.date} ${slot.time}\n${queued.length?'Nomzodlar uchun bron qilish boshlanmoqda.':'Navbatda nomzod yo‘q; bron yuborilmadi.'}`,openedShot);
          }
          this.store.saveSetting(`run:${this.runId}`,this.stats);
          for(const candidate of queued.slice(0,this.options.parallel-this.tasks.size)){
            if(!slots.length)break;
            // Spread independent sessions across the available times; a single time may have multiple places.
            const slot=[...slots].sort((a,b)=>(this.slotUse.get(a.url)||0)-(this.slotUse.get(b.url)||0))[0];
            this.slotUse.set(slot.url,(this.slotUse.get(slot.url)||0)+1);
            const task=this.book(candidate,slot).catch(()=>{this.store.event('Nomzod brauzerida xato',candidate.id);}).finally(()=>{this.tasks.delete(candidate.id);this.slotUse.set(slot.url,Math.max(0,(this.slotUse.get(slot.url)||1)-1));});
            this.tasks.set(candidate.id,task);
          }
          this.lastCalendarError=0;
          // Calendar polling discovers newly opened dates; successful forms are never refreshed.
          await this.pause(1000);
        }catch(e){
          if(e instanceof Stop)throw e;
          this.stats.errors++;this.store.saveSetting(`run:${this.runId}`,this.stats);
          if(calendar&&(!this.evidence.error||this.clock()-this.evidence.error.at>60000)){const shot=await this.capture(calendar,'error');if(shot)this.evidence.error={...shot,at:this.clock(),valid:false};}
          if(e instanceof Structure){this.store.event(e.message);this.info.message=e.message;this.store.notify(`structure:${this.runId}`,`⚠️ ${e.message}`,this.evidence.error);reason='error';break;}
          if(!this.lastCalendarError){this.store.event('Kalendar vaqtincha ochilmadi. Qayta urinish davom etadi.');this.lastCalendarError=Date.now();}
          await this.pause(3000+Math.random()*2000);
        }
      }
      ...
      if(calendar)await this.report(reason);
  ...
  async report(reason){
    ...
    if(!shots.length)this.store.notify(`report:${this.runId}`,message+'\nSayt skrinshotini olish imkoni bo‘lmadi.');
